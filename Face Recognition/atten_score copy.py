...
p = "/home/zaphieyr/Infosys/facial-landmarks-recognition/shape_predictor_68_face_landmarks.dat"
detector = dlib.get_frontal_face_detector()
landmark_predictor = dlib.shape_predictor(p)

# Create a directory to save screenshots
if not os.path.exists("Aum_screenshots"):
    os.makedirs("Aum_screenshots")

# Load the known image of Aum
known_image = face_recognition.load_image_file("aum.jpg")
...
columns = ['Name', 'Date', 'Time', 'Screenshot', 'Attentive', 'Attention Score', 'Emotion']
...
# Alert threshold for attention score
ATTENTION_THRESHOLD = 0.5  # If the attention score is below this, trigger an alert
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                attentive = 'Yes' if attention_score >= ATTENTION_THRESHOLD else 'No'
                ...
                screenshot_filename = f"Aum_screenshots/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive == 'Yes':
                ...
                cv.putText(frame, f'Emotion: {dominant_emotion}', (10, 60), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv.LINE_AA)

                # Log the recognition event with attentiveness score and emotion
                ...
                new_entry = pd.DataFrame({
                    ...
                    'Emotion': [dominant_emotion]  # Store emotion in the log
                })
                ...
                # Send alert if not attentive
                if attentive == 'No':
                    notification.notify(
                        title="Attention Alert",
                        message=f"{name} is not attentive. Attention Score: {attention_score:.2f}",
                        timeout=5
                    )
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('aum_attendance_with_attention_score_and_emotion.xlsx', index=False)
            print("DataFrame saved to 'aum_attendance_with_attention_score_and_emotion.xlsx'")
    ...
    if not df.empty:
        df.to_excel('aum_attendance_with_attention_score_and_emotion.xlsx', index=False)
        print("Final attendance with attentiveness and emotion saved to 'aum_attendance_with_attention_score_and_emotion.xlsx'.")
