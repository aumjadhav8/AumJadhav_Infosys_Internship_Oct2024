...
try:
    while True:
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('aum_attendance_with_attention_score.xlsx', index=False)
            print("DataFrame saved to 'aum_attendance_with_attention_score.xlsx'")
    ...
    if not df.empty:
        df.to_excel('aum_attendance_with_attention_score.xlsx', index=False)
        print("Final attendance with attentiveness saved to 'aum_attendance_with_attention_score.xlsx'.")
