...
try:
    while True:
        ...
        if (datetime.now() - last_save_time) > timedelta(seconds=30):
            df.to_excel('Aum_attendance_with_attention.xlsx', index=False)
            print("Data saved to 'Aum_attendance_with_attention.xlsx'")
    ...
    if not df.empty:
        df.to_excel('Aum_attendance_with_attention.xlsx', index=False)
        print("Final data saved to 'Aum_attendance_with_attention.xlsx'")
