...
entry_time = None
entry_duration = timedelta(minutes=1)  # Duration to keep the entry valid
gap_time = timedelta(minutes=2)  # Gap required to log a new entry
...
try:
    while True:
        ...
        if datetime.now() - last_save_time >= save_interval:
            df.to_excel('aum_recognized_faces.xlsx', index=False)
